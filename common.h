...
{
  char* name;
  peer_t *clientlist[10];
  int current;
};

struct group *grouplist[10];
int grouplist_current = 0;

void pv_message(peer_t *peer){
	for(int i = 0; i < sizeof(grouplist); ++i)
		for(int j = 0; j < sizeof(grouplist[i]->clientlist); ++j)
			if(peer = grouplist[i]->clientlist[j])
				send(dest_peer, data, strlen(data));
}

void group_broadcaster(peer_t *peer){
	for(int i = 0; i < sizeof(grouplist); ++i)
		for(int j = 0; j < sizeof(grouplist[i]->clientlist); ++j)
			if(peer = grouplist[i]->clientlist[j])
				for (int k = 0; k < sizeof(grouplist[i]->clientlist); ++k)
					send_to_peer(&(grouplist[i]->clientlist[k]));
}

void join_group(peer_t *peer){
	for(int i = 0; strcmp(splitter(&(peer->receiving_buffer->data)), grouplist[i]->name) != 0; ++i)
		grouplist[i]-> clientlist[grouplist[i]->current++];
}

void create_group(peer_t *peer){
  struct group new_group;
  new_group.current = 0;
  new_group.name = splitter(&(peer->receiving_buffer->data));
  new_group.clientlist[new_group.current++] = peer;
  grouplist[grouplist_current++] -> new_group;
}

void command_handler(peer_t *peer){
  if(strcmp(peer->receiving_buffer->data, "grouplist") == 0)
    get_grouplist();
  else if(StartsWith(peer->receiving_buffer->data, "create_group"))
    create_group();
  else if(StartsWith(peer->receiving_buffer->data, "join_group"))
    join_group(&peer);
	else if (StartsWith(peer->receiving_buffer->data, "join_group"))
	pv_message();
}
